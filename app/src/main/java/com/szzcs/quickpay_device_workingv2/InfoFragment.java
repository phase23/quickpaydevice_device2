...
public class InfoFragment extends PreferenceFragment {
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                // pid
                int resPid = mSys.getSN(mTerminalSN);
                Log.i(TAG, "getSN res: " + resPid);
                if (resPid == SdkResult.SDK_OK) {

                    Log.i(TAG, "getSN: " + mTerminalSN[0]);
                ...
                if (resSN == SdkResult.SDK_OK) {
                    ...
                    String ele = new String(new byte[]{(byte) 0xFF});
                    // 默认隐藏乱码显示
                    if (mCustomerSN[0].startsWith(ele) && mCustomerSN[0].endsWith(ele)) {
                        mCustomerSN[0] = "";
                    }
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        findPreference(getString(R.string.key_upload_log)).setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            @Override
            public boolean onPreferenceClick(Preference preference) {
                Intent intent = new Intent("com.zcs.zsf.UPLOAD_LOG")
                        .setComponent(new ComponentName("com.zcs.zsf", "com.zcs.zsf.core.CustomReceiver"));
