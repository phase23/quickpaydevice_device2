...
public class LoyaltyFragment extends PreferenceFragment {
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        addPreferencesFromResource(R.xml.pref_loyalty);
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        findPreference(getString(R.string.key_registercard)).setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            @Override
            public boolean onPreferenceClick(Preference preference) {
                setwhatclick("register");
                searchM1();
                return true;
            }
        });

        findPreference(getString(R.string.key_addpoints)).setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            @Override
            public boolean onPreferenceClick(Preference preference) {
                setwhatclick("addpoints");
                searchM1();
                return true;
            }
        });

        findPreference(getString(R.string.key_viewpoints)).setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            @Override
            public boolean onPreferenceClick(Preference preference) {
                setwhatclick("viewpoints");
                searchM1();
                return true;
            }
        });
        ...
        findPreference(getString(R.string.key_at24)).setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            @Override
            public boolean onPreferenceClick(Preference preference) {
        ...
        findPreference(getString(R.string.key_at102)).setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            @Override
            public boolean onPreferenceClick(Preference preference) {
        ...
        findPreference(getString(R.string.key_at1608)).setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            @Override
            public boolean onPreferenceClick(Preference preference) {
        ...
        findPreference(getString(R.string.key_at153)).setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            @Override
            public boolean onPreferenceClick(Preference preference) {
        ...
        findPreference(getString(R.string.key_nfc)).setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            @Override
            public boolean onPreferenceClick(Preference preference) {
    ...
    public String getwhatclick() {
        return coreclick;
    }

    public void setwhatclick(String thisaction) {
        this.coreclick = thisaction;
    }
    ...
    private void readMagCard() {
        ...
        if (cardInfo.getResultcode() == SdkResult.SDK_OK) {
            String expiry = cardInfo.getExpiredDate();
            String cardNo = cardInfo.getCardNo();
            String carddetails = cardInfo.getTk1();
            ...
            Log.i("[print]",  "scan details" + carddetails);
            Log.i("[print]",  "scan no" + cardNo);
            Log.i("[print]",  "scan expr" + expiry);
    ...
    private void readM1Card() {
        ...
        do {
            // sector 10 = 4 * 10
            status = mRfCard.m1VerifyKey((byte) (4 * 0), keyType, key);
            ...
            for (int i = 0; i < 1; i++) {
                ...
                status = mRfCard.m1ReadBlock((byte) (4 * 0 + i), out);
                if (status == SdkResult.SDK_OK) {
                    m1_message.append("\nBlock").append(i).append(":")
                            .append(StringUtils.convertBytesToHex(out));
                    m2_message = StringUtils.convertBytesToHex(out);

                    Log.d("print/",  m2_message);
                    String myuid = m2_message.substring(0,8);
                    Log.d("print/",  myuid);

                    String littleedian = reverseHex(myuid);
                    Log.d("print/",  littleedian);
                    BigInteger decimal = new BigInteger(littleedian, 16);
                    System.out.println("print/: " + decimal);
                    String struid = decimal.toString();
                    String whataction = getwhatclick();

                    if(whataction.equals("register")) {
                        Intent intent = new Intent(getActivity(), registerCard.class);
                        intent.putExtra("UID", struid);
                        startActivity(intent);
                    }

                    if(whataction.equals("addpoints")) {
                        Intent intent = new Intent(getActivity(), Addpoints.class);
                        intent.putExtra("UID", struid);
                        startActivity(intent);
                    }

                    if(whataction.equals("viewpoints")) {
                        Intent intent = new Intent(getActivity(), Viewpoints.class);
                        intent.putExtra("UID", struid);
                        startActivity(intent);
                    }
 ...
 */
    ...
    class CardHandler extends Handler implements DialogInterface.OnClickListener, DialogInterface.OnCancelListener {
        ...
        public void handleMessage(Message msg) {
            LoyaltyFragment fragment = (LoyaltyFragment) mFragment.get();
            ...
            switch (msg.what) {
                ...
                case MSG_CARD_APDU:
                    mCardInfoDialog = DialogUtils.show(fragment.getActivity(),
                            ...
                            SDK_Result.appendMsg("Code", msg.arg1 + "", "APDU send", msg.getData().getString(LoyaltyFragment.KEY_APDU), "APDU response", (String) msg.obj),
                            "OK", this, this);
                    break;
                case MSG_RF_CARD_APDU:
                    ...
                    mCardInfoDialog = DialogUtils.show(fragment.getActivity(), type,
                            SDK_Result.appendMsg("Code", msg.arg1 + "", "Send", msg.getData().getString(LoyaltyFragment.KEY_APDU), "Response", (String) msg.obj),
                            "OK", this, this);
                    break;
                case MSG_CARD_M1:
                    mCardInfoDialog = DialogUtils.show(fragment.getActivity(),
