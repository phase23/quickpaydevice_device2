...
import com.szzcs.quickpay_device_workingv2.qr.QRTestActivity;
import com.szzcs.quickpay_device_workingv2.scan.PermissionsManager;
import com.szzcs.quickpay_device_workingv2.utils.Config;
import com.szzcs.quickpay_device_workingv2.utils.DialogUtils;
import com.szzcs.quickpay_device_workingv2.utils.FeatureSupport;
import com.szzcs.quickpay_device_workingv2.utils.FilePickerUtils;
import com.szzcs.quickpay_device_workingv2.utils.Kits;
import com.szzcs.quickpay_device_workingv2.utils.SDK_Result;
import com.szzcs.quickpay_device_workingv2.utils.SystemInfoUtils;
import com.szzcs.quickpay_device_workingv2.utils.update.UpdateBuilder;
import com.szzcs.quickpay_device_workingv2.utils.update.UpdateConfig;
import com.szzcs.quickpay_device_workingv2.utils.update.base.FileChecker;
import com.szzcs.quickpay_device_workingv2.utils.update.base.FileCreator;
import com.szzcs.quickpay_device_workingv2.utils.update.base.InstallStrategy;
import com.szzcs.quickpay_device_workingv2.utils.update.base.UpdateChecker;
import com.szzcs.quickpay_device_workingv2.utils.update.base.UpdateParser;
import com.szzcs.quickpay_device_workingv2.utils.update.model.CheckEntity;
import com.szzcs.quickpay_device_workingv2.utils.update.model.Update;
...
public class SettingsFragment extends PreferenceFragment {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        findPreference(getString(R.string.key_setting)).setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            @Override
            public boolean onPreferenceClick(Preference preference) {

                Intent intent = new Intent(getActivity(), Linkdevice.class);
        ...
        findPreference(getString(R.string.key_email)).setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            @Override
            public boolean onPreferenceClick(Preference preference) {
                Intent intent = new Intent(getActivity(), Emailinvoice.class);
                startActivity(intent);
                return true;
            }
        });
        ...
        findPreference(getString(R.string.key_loaylty_points)).setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            @Override
            public boolean onPreferenceClick(Preference preference) {
                title = getString(R.string.pref_card);
                switchFragment(SettingsFragment.this, new LoyaltyFragment());
        ...
        findPreference(getString(R.string.key_subscription)).setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            @Override
            public boolean onPreferenceClick(Preference preference) {
...
/*
        ...
        */
        ...
        findPreference(getString(R.string.key_baudrate)).setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener() {
            @Override
            public boolean onPreferenceChange(Preference preference, Object newValue) {
                int index = ((ListPreference) preference).findIndexOfValue((String) newValue);
                int ret = 0;
                try {
                    Method m = mSys.getClass().getDeclaredMethod("a", byte.class);
                    m.setAccessible(true);
                    ret = (int) m.invoke(mSys, (byte) index);
        ...
        // print sn and barcode
        findPreference(getString(R.string.key_print_sn)).setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            @Override
            public boolean onPreferenceClick(Preference preference) {
                printSN();
                return true;
        ...
        findPreference(getString(R.string.key_test_spi)).setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            @Override
            public boolean onPreferenceClick(Preference preference) {
                testSpi();
                return true;
    ...
    void testSpi() {
        final ProgressDialog dialog = ProgressDialog.show(mActivity, "", "looping", false, true);
    ...
    private void setDeviceInfo() {
        ...
        if (TextUtils.isEmpty(imei1)) {
            mActivity.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    DialogUtils.show(mActivity, "Warning! No IMEI!!!");
                }
            });
            return;
        }
    ...
    private void printSN() {
        ...
        int ret = mSys.getSN(sn);
        if (ret != SdkResult.SDK_OK || "FFFFFFFFFFFFFFFF".equals(sn[0])) {
            Toast.makeText(mActivity, R.string.msg_print_sn, Toast.LENGTH_SHORT).show();
            return;
        }
        // sn
        PrnStrFormat psf = new PrnStrFormat();
        psf.setTextSize(25);
        printer.setPrintAppendString("SN: " + sn[0], psf);
        printer.setPrintAppendBarCode(mActivity, sn[0], 300, 100, false, Layout.Alignment.ALIGN_CENTER, BarcodeFormat.CODE_128);
        printer.setPrintAppendString("", psf);
        // imei1
        Map<String, String> map = SystemInfoUtils.getImeiAndMeid(mActivity.getApplicationContext());
        String imei1 = map.get("imei1");
        String imei2 = map.get("imei2");
        if (!TextUtils.isEmpty(imei1)) {
            printer.setPrintAppendString("IMEI1: " + imei1, psf);
            printer.setPrintAppendBarCode(mActivity, imei1, 360, 100, false, Layout.Alignment.ALIGN_CENTER, BarcodeFormat.CODE_128);
            printer.setPrintAppendString("", psf);
        }
        // imei2
        if (!TextUtils.isEmpty(imei2)) {
            printer.setPrintAppendString("IMEI2: " + imei2, psf);
            printer.setPrintAppendBarCode(mActivity, imei2, 360, 100, false, Layout.Alignment.ALIGN_CENTER, BarcodeFormat.CODE_128);
